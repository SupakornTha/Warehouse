class warehouse:
    ...
    def string2code(self,string):
        ...
        if len(b) == 2:
            ...
            b = "00" + b
        product=(a+str(d)+str(b))
        return product
    def insert(self,data1,position):
        a=int((ord(data1[0]) % 65) / 5)
        b=int(data1[1])-1
        if  (self.row[b][a].get(data1)==None)\
            and (self.ware[int(position[0])][int(position[1:3])].get(position)==None):
    ...
    def findposition(self, string):
        ...
        if len(string[1:]) == 3:
            ...
            if len(b) == 1:
                b = "0" + b
        string1= a + b + string[3:]
        if self.ware[int(string1[0])][int(string1[1:3])].get(string1)!=None:
            return False
        else:
            return True
    ...
    def sort(self,x,y):
        ...
        for i in self.ware[x-1][y-1]:
            if bool(self.ware[x-1][y-1][i]==i)==False:
    ...
    def info(self):
        ...
        print ("  C  represents a number of slot.It has a value of 000 to 399.")
    def data(self):
    ...
    def command(self):
        ...
        if 4<len(a)<7:
                ...
                if self.checkname(a[1:]) == False or len(a) != 5:
                    ...
                    if self.findposition(c) == False:
                        ...
                        if len(self.ware[int(c[0])][int(c[1:3])])<self.slotnum[int(c[0])]:
                            ...
                            print("Slot is occupied. Cannot store the product.")
                        return self.command()
            elif a[0] == "2" and  a[len(a)-2:]=="00" and 49<=ord(a[1])<=53 and 48<=int(a[len(a)-3])<=57 and 48<=ord(a[2])<=57 :
                if int(a[2:len(a) - 2]) > self.rownum[int(a[1]) - 1]:
            ...
            if a[0] == "9"  and (65<=ord(a[1])<=89 or 97<=ord(a[1])<=121) and 49<=ord(a[2])<=53 and 48<=ord(a[6])<=50 and 48<=ord(a[7])<=57 and 48<=ord(a[8])<=51 and 48<=ord(a[9])<=57 and 48<=ord(a[10])<=57 and 48<=ord(a[3])<=57 and 48<=ord(a[4])<=57 and 49<=ord(a[5])<=53 and int(a[6:8])<=self.rownum[int(a[5])-1] and int(a[8:]) < self.slotnum[int(a[5])-1] :
                if a[5] == "0":
                    print("Invalid command.")
                    return self.command()
